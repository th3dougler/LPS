...
function ready() {
  ...
  player.buttons.forEach(function (e) {
...
function adjustBtns(clickedId, buttonSet) {
  ...
  for (i = 0; i < l; i++) {
    if (clickedId == null) buttonSet[i].classList.remove("blocked", "choice");
    else if (clickedId == i) buttonSet[i].classList.add("choice");
    else buttonSet[i].classList.add("blocked");
  }
}
...
function countDown(iteration) {
  if (iteration != 0) {
    ...
    document.querySelector(".results").innerText = comparePlays(
